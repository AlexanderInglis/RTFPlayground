...
import { Image, ScrollControls, Scroll, useScroll, MeshDistortMaterial } from '@react-three/drei'
...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
    ...
    useFrame((state, delta) => {
        ...
        //scaling on Scroll
        group.current.position.z = THREE.MathUtils.damp(group.current.position.z, Math.max(0, data.delta * 10), 4, delta)

        //Chromatic Aberration on Scroll
        chrome.current.offset.x = THREE.MathUtils.damp(group.current.position.z, Math.max(0, data.delta * 0.0000001), 325, delta)
...
function Items({ w = 0.7, gap = 0.15 }) {
    ...
    return (
        <ScrollControls horizontal damping={4} offset={2} pages={(width - xW + urls.length * xW) / width}>
            <Minimap />
            <Scroll>
